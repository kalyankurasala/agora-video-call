...
function addView(id, show) {
    if (!$("#" + id)[0]) {
        ...
        remoteContainer = document.getElementById("remote_video_panel_" + id);
    }
}
...
function join(rtc, option) {
    ...
    **/
    ...
    // init client
    rtc.client.init(config.appID, function () {
        ...
        **/
        rtc.client.join(config.token ? config.token : null, option.channel, option.uid ? +option.uid : null, function (uid) {
...
var config = {
    appID: '308a500f53e249e4adf0e711dc201838',
    channel: null,
    uid: null,
    token: null
};
...
$(function () {
    ...
    $.ajax({
        url: "http://localhost:8081/rtcToken?channelName=1234",
        success: function (result) {
            config.token = result.key;
        }
    });
...
let remoteContainer;// = document.querySelector('[id^="remote-video-panel-"]').id;//document.getElementById("remote-container");
let canvasContainer = document.getElementById("local_stream");//document.getElementById("canvas-container");
...
function addCanvas(streamId) {
    ...
    let video = document.getElementById(`remote_video_${streamId}`);
